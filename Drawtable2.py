    ...
    Attributes:
        ...
        **kwargs
            ...
            Custom text style:
                ...
                first_column_header(bool)(default: False) first column use the font for the header
    """
            ...
            try:
                ...
                for row_idx,row in enumerate(self.data):
                    ...
                    for num_columns,j in enumerate(row):
                        col_width = (self.__width_per_cell[num_columns])-(self.margin)
                        multilines = j.split('\n')
                        lines = []
                        ...
                        for idx,line in enumerate(lines): 
                            if (row_idx==0 and self.header_frame) or ( num_columns==0 and self.first_column_header) or (self.alternate_bold > 0 and row_idx % self.alternate_bold == 0 ):
                                ...
                                x,y,width, height = self.font.getbbox(line)
                                xy=self.x_init+self.margin+(sum(self.__width_per_cell[:num_columns])),cur_h
