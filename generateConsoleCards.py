...
def draw_multiline_textbox(bg_image, string, font: ImageFont.FreeTypeFont, x, y, width, height, color='black', edge=False, h_align="center", v_align="center"):
...
def create_item_card(item_info, output_path="item_card.png"):
	...
	font_size = 18*3
	# width = 1180 + 1180/2
	card_width, card_height = 1772, 1180
	...
	card.paste(qr_img, (card_width-qr_img.width-gap, card_height-qr_img.height-gap), qr_img)

	#FonteWiki
	draw_multiline_textbox( 
		card, "(2023) Wikipedia", font_small,
		int(card_width * 2/3 + gap), int(card_height * 4/5 + gap),
		int(card_width * 1/3 - qr_img.width - 3*gap), int(card_height * 1/5 -2*gap),
		color=text_color, v_align="bottom", h_align="right"
	)
	...
	if(item_info['Produttore']):
		tdata.append(('Produttore',item_info['Produttore']))

	if(item_info['Generazione']):
		tdata.append(('Generazione',item_info['Generazione']))
	...
	if(item_info['Anno di rilascio']):
		...
		tdata.append(('In vendita', from_to_sell))

	if(item_info['Unita vendute']):
		tdata.append(('Unità vendute',item_info['Unita vendute']))
	
	if(item_info['CPU']):
		tdata.append(('Processore',item_info['CPU']))

	if(item_info['RAM totale']):
		tdata.append(('RAM',item_info['RAM totale']))
	
	if(item_info['GPU']):
		tdata.append(('GPU',item_info['GPU']))

	if(item_info['Supporto di memoria']):
		tdata.append(('Supporti', item_info['Supporto di memoria']))

	if(item_info['Dispositivi di controllo']):
		tdata.append(('Controller', item_info['Dispositivi di controllo']))

	if(item_info['Gioco più diffuso']):
		tdata.append(('Bestseller',item_info['Gioco più diffuso']))
...
def generate_cards(input_file):
   ...
   for item_info in item_infos:
       output_path = f"cards/{item_info['Nome'].replace('/','-')}_card.png"
